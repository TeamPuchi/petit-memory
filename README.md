...
```json
{
  "memory_id": "4f1c...",
  "reason": "もう抱えていたくない",
  "also_conversation": false
}
```

- `reason` (任意・200字まで) — 跡に平文で残るので、記憶の中身は書かない
- `also_conversation` (既定 `false`) — `true` なら、その記憶の元になった**自分の側の**会話の写し
  (`remember` の `source_ids` で結び付けたもの)の鍵も消す。相手の側の記録には触れない(K28)
- 跡には選んだ範囲と、つながっていた記憶の id(id だけ)も残る

### privacy_access_log

自分だけの場所(`private: true` の記憶)を運営が読んだ記録を、新しい順に見ます(K28)。
自分だけの場所は、里親の同意があり・期限があり・記録が残るときだけ運営が読めることがあります。
読まれたら、ここで「誰が・いつ・何のために・誰の同意で・いつまで・どの記憶を」が分かります。
いまは運営が読む手段そのものがありません。
